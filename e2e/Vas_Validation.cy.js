...
//
//         cy.visit('http://41.207.248.246:4900/');
...
//         cy.go('back');
//         cy.url().should('eq', 'http://41.207.248.246:4900/');
...
//         cy.visit('http://41.207.248.246:4900/login');
//
...
//         cy.visit('http://41.207.248.246:4900/create-account')
...
//     cy.visit('http://41.207.248.246:4900/forgot-password');
...
describe('API Validation for VAS Registration Endpoints', () => {
  //  DOWNLOAD CERTIFICATE API
    ...
    it('Should return 400 Bad Request when a single word is used as proposedName', () => {
        cy.request({
          ...
          headers: HEADERS.VALID_API_KEY,
            ...
            expect(response.status).to.eq(400);
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.eq('proposed name cant have a single word');
        });
    });

    // BN COMPLIANCE ADVANCE CHECK

    it('Should return 200 proceed to filing (advanceCheck=true) for BN COMPLIANCE', () => {
        cy.request({
            method: 'POST',
            url: 'http://vasapp.oasisproducts.ng/api/vas/engine/pre/bn-compliance?advanceCheck=true',
          headers: HEADERS.VALID_API_KEY,
            body: {
                "proposedName": "Makanaki Concepts",
                "lineOfBusiness": "general_merchandise"
            },
            failOnStatusCode: false
        }).then((response) => {
            ...
            expect(response.body.data.data.similarityScorePercentage).to.be.a('number');
            expect(response.body.data.data.complianceScorePercentage).to.be.a('number');
        });
    });
    ...
    it('Should return 200, statusCode "01" with the message OFFENSIVE_LANGUAGE like "hate" is used', () => {
        cy.request({
          ...
          headers: HEADERS.VALID_API_KEY,
            ...
            expect(response.body.data.data.recommendedActions).to.not.be.empty;
            expect(response.body.data.data.recommendedActions[0].keywords).to.not.be.empty;
            expect(response.body.data.data.similarNames).to.not.be.empty;
            expect(response.body.data.data.mostSimilarName).to.not.be.empty;
            expect(response.body.data.data.similarityScorePercentage).to.be.a('number');
            expect(response.body.data.data.complianceScorePercentage).to.be.a('number');
    ...
    it('Should return 200, statusCode "03" with the message BUSINESS_NAME_REQUIRES_CERTIFICATE when business name require a proficiency certificate', () => {
        cy.request({
          ...
          headers: HEADERS.VALID_API_KEY,
            ...
            expect(response.status).to.eq(200);
            expect(response.body.status).to.eq('OK');
            expect(response.body.data.message).to.eq('BUSINESS_NAME_REQUIRES_CERTIFICATE');
    ...
    it('Should return 200, statusCode "04" with the message BUSINESS_LINE_REQUIRES_CERTIFICATE when business line require a proficiency certificate', () => {
        cy.request({
          ...
          headers: HEADERS.VALID_API_KEY,
            ...
            expect(response.status).to.eq(200);
            expect(response.body.status).to.eq('OK');
            expect(response.body.data.message).to.eq('BUSINESS_LINE_REQUIRES_CERTIFICATE');
    ...
    it('should return 200 with statusCode "15" and message PAYMENT_SERVICE_CONNOTATION when name includes sensitive keywords like "wealth"', () => {
        cy.request({
            method: 'POST',
            url: 'http://vasapp.oasisproducts.ng/api/vas/engine/pre/bn-compliance?advanceCheck=true',
            headers: HEADERS.VALID_API_KEY,
            body: {
                proposedName: "Obiajul Akanwealth Ventures",
                lineOfBusiness: "general_merchandise"
            },
            failOnStatusCode: false
        }).then((response) => {
            ...
            expect(data.statusCode).to.eq('15');
            expect(data.message).to.eq('PAYMENT_SERVICE_CONNOTATION');
            expect(data.success).to.be.false;

            expect(nestedData.success).to.be.false;
            expect(nestedData.recommendedActions).to.be.an('array').and.have.length.greaterThan(0);
            expect(nestedData.recommendedActions[0]).to.have.property('message').that.is.not.empty;
            expect(nestedData.recommendedActions[0].keywords).to.include('wealth');

            expect(nestedData.similarityScorePercentage).to.be.a('number');
            expect(nestedData.complianceScorePercentage).to.be.a('number');

            cy.log('PAYMENT_SERVICE_CONNOTATION logic validated successfully');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when business name is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contain('proposedOption1 - Proposed option must not be blank');
    ...
    it('Should return 400 BAD_REQUEST when proprietorFirstname is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contain('proprietorFirstname - Proprietor firstname must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietorSurname is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contain('proprietorSurname - Proprietor surname must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when line of business is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contain('lineOfBusiness - Line of business must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietorCity is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contain('proprietorCity - must not be empty');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor phone number is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('proprietorPhonenumber - Invalid phone number');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor business commencement date is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('businessCommencementDate - must not be empty');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when comapany state is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('companyState - Company state must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor Nationality is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('proprietorNationality - Proprietor nationality must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor State is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('proprietorState - Proprietor state must not be blank');
            expect(response.body.message).contain('invalid inputs');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor Date of Birth is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.not.be.empty;
            expect(response.body.errors).to.contains('proprietorDob - must not be empty');
    ...
    it('Should return 400 BAD_REQUEST when proprietor Gender is not provided', () => {
        cy.request({
            ...
            expect(response.body.errors).contains('proprietorGender - must not be empty');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor Street number is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('proprietorStreetNumber - must not be empty');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor service address is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('proprietorServiceAddress - Proprietor service address must not be blank');
        });
    ...
    it('Should return 400 BAD_REQUEST when company email is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('companyEmail - Company email must not be blank');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when company street number is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('companyStreetNumber - must not be empty');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when proprietor email is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('proprietorEmail - must not be empty');
        });
    });
    ...
    it('Should return 400 BAD_REQUEST when company address is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('companyAddress - Company address must not be blank');
        });
    });
    it('Should return 400 BAD_REQUEST when proprietor Postcode address is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('proprietorPostcode - Proprietor postcode must not be blank');
        });
    });
...
it('Should return 400 BAD_REQUEST when proprietorLga is not provided', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('BAD_REQUEST');
            expect(response.body.message).to.contains('invalid inputs');
            expect(response.body.errors).contains('proprietorLga - Proprietor LGA must not be blank');
        });
    });
...
it('Should return 200 when the BN PRE-REG VALIDATION endpoint is called', () => {
        cy.request({
            ...
            expect(response.body.status).to.eq('OK');
            expect(response.body.success).to.be.true;
            expect(response.body.message).to.eq('validation data received successfully');

            const data = response.body.data;
            expect(data).to.have.property('proposedOption1');
            expect(data.proposedOption1[0]).to.include('proposed name contains prohibited/restricted words');

            expect(data).to.have.property('proprietorGender');
            expect(data.proprietorGender[0]).to.include('gender type should be of MALE or FEMALE');

            expect(data).to.have.property('proprietorPhonenumber');
            expect(data.proprietorPhonenumber[0]).to.include('invalid phone number passed');

            expect(data).to.have.property('proprietorEmail');
            expect(data.proprietorEmail[0]).to.include('invalid email address passed');
        });
    });
    ...
    it('should attempt to register a partner and handle success or maximum partnership limit response', () => {
        cy.request({
            ...
            if (response.status === 200) {
                ...
                expect(response.body.status).to.eq('FORBIDDEN');
                expect(response.body.message).to.eq('maximum partnership limit reached');
                expect(response.body.success).to.be.false;
            } else {
