...
describe("LLC Corporate Registration API Flow", () => {
  let reservationCode;
  let transactionRef;
  ...
  it("should return 200 and reservation details when a unique proposedName is submitted", () => {
    const proposedName = `FridayLLC${Date.now()} Academy Enterprise`;
  ...
  // 4. REGISTER AFFILIATE - FOREIGN 1st
  it("should register Foreign affiliate using the same transactionRef AS 1st Affiliate", () => {
    cy.request({
      method: "POST",
      ...
      body: {
          ...
          corporate: {
              isForeign: false,
              rcNumber: "80345",
              companyName: "BENTON FARMS NIGERIA LTD",
      ...
      expect(data.nextStepUrl).to.be.a("string");
      expect(data.nextStepUrl).to.not.be.empty;
      cy.log(`1st Affiliate Key: ${affiliateKeyCorporate}`);
    });
  });
